...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	return (runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight * duration * minInH / mInKm
}
...
// powMeanSpeedMInSec — квадрат средней скорости в м/с
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	powMeanSpeedMInSec := math.Pow((meanSpeed(action, duration))*kmhInMsec, 2)
	return (walkingCaloriesWeightMultiplier*weight + (powMeanSpeedMInSec/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
